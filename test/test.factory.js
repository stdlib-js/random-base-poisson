...
tape( 'attached to the returned function is the generator name', function test( t ) {
	var poisson = factory();
	t.strictEqual( poisson.NAME, 'poisson', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var poisson = factory();
	t.strictEqual( typeof poisson.PRNG, 'function', 'has property' );

	poisson = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( poisson.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( isUint32Array( poisson.seed ), true, 'has property' );
	t.strictEqual( poisson.seed[ 0 ], 12345, 'equal to provided seed' );

	poisson = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( poisson.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var poisson = factory();
	t.strictEqual( typeof poisson.seedLength, 'number', 'has property' );

	poisson = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( poisson.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state', function test( t ) {
	var poisson = factory();
	t.strictEqual( isUint32Array( poisson.state ), true, 'has property' );

	poisson = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( poisson.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var poisson = factory();
	t.strictEqual( typeof poisson.stateLength, 'number', 'has property' );

	poisson = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( poisson.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var poisson = factory();
	t.strictEqual( typeof poisson.byteLength, 'number', 'has property' );

	poisson = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( poisson.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	poisson = factory();
	t.strictEqual( typeof poisson.toJSON, 'function', 'has method' );

	o = poisson.toJSON();
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, poisson.NAME, 'has property' );
	...
	t.strictEqual( typeof poisson.toJSON, 'function', 'has method' );
	t.strictEqual( poisson.toJSON(), null, 'returns expected value' );
...
tape( 'the function supports specifying the generator state (lambda < 30)', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( poisson(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
